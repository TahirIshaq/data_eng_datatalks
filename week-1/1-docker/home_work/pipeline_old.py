...
def get_args():
    ...
    parser.add_argument("--username", default="admin", help="DB username")
    ...
    parser.add_argument("--compression", default=None, help="Dataset compression")
...
def load_dataset(name, nrows=None, iterator=False, chunksize=None):
    ...
    """
    df = pf.read_csv(name, nrows=nrows, iterator=iterator, chunksize=chunksize)
    return df
...
def update_datatype(df, col_name, datatype):
    ...
    """
    for data in range(len(col_name)):
...
def insert_in_database(df, table_name, con, operation):
    ...
    """
    df.to_sql(table_name=table_name, con=con, if_exists=operation)
